...
class HookExampleGenerator {
  ...
  async generateHookExamples(projectPath, selectedHookIds) {
    ...
    const hooks = allHookIds.map((id) => getHookById(id)).filter(Boolean);
    const examplesDir = path__namespace.join(projectPath, "Pages", "Examples");
    await fs__namespace.mkdir(examplesDir, { recursive: true });
  ...
  async generateHookExample(examplesDir, hook) {
    ...
    const filePath = path__namespace.join(examplesDir, fileName);
  ...
  getExampleFileName(hookName) {
    const pascalCase = hookName.replace(/^use/, "Use").replace(/([A-Z])/g, "$1").replace(/^./, (str) => str.toUpperCase());
...
`;
  ...
  async generateExamplesIndex(examplesDir, hooks) {
    const indexPath = path__namespace.join(examplesDir, "Index.tsx");
    ...
    await fs__namespace.writeFile(indexPath, content, "utf-8");
  }
  /**
...
${hookElements.join("\n\n")}
  ...
  async copyRequiredJsFiles(projectPath, selectedHookIds) {
    ...
    const jsDir = path__namespace.join(projectPath, "wwwroot", "js");
    await fs__namespace.mkdir(jsDir, { recursive: true });
    const mactModulesDir = path__namespace.join(__dirname, "..", "..", "mact_modules", "@minimact");
    for (const packageName of requiredPackages) {
      const shortName = packageName.replace("@minimact/", "");
      try {
        const sourcePath = path__namespace.join(mactModulesDir, shortName, "dist", `${shortName}.min.js`);
        const exists = await fs__namespace.access(sourcePath).then(() => true).catch(() => false);
        if (!exists) {
        ...
        const destPath = path__namespace.join(jsDir, `minimact-${shortName}.min.js`);
        await fs__namespace.copyFile(sourcePath, destPath);
  ...
  async generateServerCode(projectPath, hook) {
    ...
    const targetDir = hook.serverCode.language === "csharp" ? path__namespace.join(projectPath, "ServerReducers") : path__namespace.join(projectPath, "ServerReducers");
    ...
    const filePath = path__namespace.join(targetDir, hook.serverCode.fileName);
    await fs__namespace.writeFile(filePath, code, "utf-8");
...
class ProjectManager {
  ...
  async createProjectStructure(projectPath, projectName, template, enableTailwind = false) {
    ...
    if (template === "Dashboard" || template === "MVC-Dashboard") {
      await execa2("dotnet", ["add", "package", "Minimact.Charts"], {
        cwd: projectPath
      });
      await execa2("dotnet", ["add", "package", "Minimact.Powered"], {
        cwd: projectPath
      });
...
`;
  ...
  async createCounterTemplate(projectPath) {
    await fs__namespace.mkdir(path__namespace.join(projectPath, "Controllers"), { recursive: true });
    await fs__namespace.mkdir(path__namespace.join(projectPath, "wwwroot"), { recursive: true });
    const homeControllerCs = `using Microsoft.AspNetCore.Mvc;
using Minimact.AspNetCore.Rendering;

namespace ${path__namespace.basename(projectPath)}.Controllers;
...
`;
    await fs__namespace.writeFile(
      path__namespace.join(projectPath, "Controllers", "HomeController.cs"),
      homeControllerCs,
      "utf-8"
    );
...
`;
    await fs__namespace.writeFile(path__namespace.join(projectPath, "Pages", "CounterPage.tsx"), counterPageTsx, "utf-8");
...
`;
    await fs__namespace.writeFile(path__namespace.join(projectPath, "Pages", "TodoListPage.tsx"), todoListPageTsx, "utf-8");
    await this.copyClientRuntimeToProject(projectPath);
  }
  /**
...
`;
    await fs__namespace.writeFile(path__namespace.join(projectPath, "Pages", "DashboardPage.tsx"), dashboardPageTsx, "utf-8");
    await this.copyClientRuntimeToProject(projectPath);
  ...
  async createMvcDashboardTemplate(projectPath) {
    await fs__namespace.mkdir(path__namespace.join(projectPath, "Controllers"), { recursive: true });
    await fs__namespace.mkdir(path__namespace.join(projectPath, "ViewModels"), { recursive: true });
    await fs__namespace.mkdir(path__namespace.join(projectPath, "wwwroot"), { recursive: true });
    const dashboardViewModelCs = `using Minimact.AspNetCore.Attributes;

namespace ${path__namespace.basename(projectPath)}.ViewModels;
...
`;
    await fs__namespace.writeFile(
      path__namespace.join(projectPath, "ViewModels", "DashboardViewModel.cs"),
      dashboardViewModelCs,
      "utf-8"
    );
    const dashboardControllerCs = `using Microsoft.AspNetCore.Mvc;
using Minimact.AspNetCore.Rendering;
using ${path__namespace.basename(projectPath)}.ViewModels;
...
`;
    await fs__namespace.writeFile(
      path__namespace.join(projectPath, "Controllers", "DashboardController.cs"),
...
`;
    await fs__namespace.writeFile(
      path__namespace.join(projectPath, "Pages", "DashboardPage.tsx"),
      dashboardPageTsx,
      "utf-8"
    );
    await this.copyClientRuntimeToProject(projectPath);
...
`;
    await fs__namespace.writeFile(
      path__namespace.join(projectPath, "ViewModels", "ProductViewModel.cs"),
      productViewModelCs,
      "utf-8"
    );
...
`;
    await fs__namespace.writeFile(
      path__namespace.join(projectPath, "Controllers", "ProductsController.cs"),
    ...
    await fs__namespace.writeFile(
      path__namespace.join(projectPath, "Pages", "ProductDetailsPage.tsx"),
  ...
  async copyClientRuntimeToProject(projectPath) {
    const jsDir = path__namespace.join(projectPath, "wwwroot", "js");
    await fs__namespace.mkdir(jsDir, { recursive: true });
    const clientRuntimeSource = path__namespace.join(__dirname, "..", "..", "resources", "minimact.js");
    const clientRuntimeDest = path__namespace.join(jsDir, "minimact.js");
    try {
      await fs__namespace.copyFile(clientRuntimeSource, clientRuntimeDest);
    } catch (error) {
  ...
  async copyChartPackagesToProject(projectPath) {
    const jsDir = path__namespace.join(projectPath, "wwwroot", "js");
    await fs__namespace.mkdir(jsDir, { recursive: true });
    const chartsSource = path__namespace.join(__dirname, "..", "..", "mact_modules", "@minimact", "charts", "dist", "charts.js");
    const chartsDest = path__namespace.join(jsDir, "minimact-charts.min.js");
    try {
      await fs__namespace.copyFile(chartsSource, chartsDest);
  ...
  async copyPoweredPackageToProject(projectPath) {
    const jsDir = path__namespace.join(projectPath, "wwwroot", "js");
    await fs__namespace.mkdir(jsDir, { recursive: true });
    const poweredSource = path__namespace.join(__dirname, "..", "..", "mact_modules", "@minimact", "powered", "dist", "powered.js");
    const poweredDest = path__namespace.join(jsDir, "minimact-powered.min.js");
    try {
      await fs__namespace.copyFile(poweredSource, poweredDest);
  ...
  async createSolutionFile(projectPath, projectName) {
    const { execa: execa2 } = await import("execa");
    try {
      await execa2("dotnet", ["new", "sln", "-n", projectName], {
        cwd: projectPath
      });
      const csprojFile = `${projectName}.csproj`;
      await execa2("dotnet", ["sln", "add", csprojFile], {
        cwd: projectPath
      });
  ...
  async setupTailwindCss(projectPath, projectName) {
    ...
    try {
      ...
      const packageJsonExists = await fs__namespace.access(packageJsonPath).then(() => true).catch(() => false);
      if (!packageJsonExists) {
        ...
        await fs__namespace.writeFile(packageJsonPath, JSON.stringify(packageJson, null, 2), "utf-8");
...
`;
      await fs__namespace.writeFile(path__namespace.join(projectPath, "tailwind.config.js"), tailwindConfig, "utf-8");
      console.log("[ProjectManager] Created tailwind.config.js");
      const stylesDir = path__namespace.join(projectPath, "src", "styles");
      await fs__namespace.mkdir(stylesDir, { recursive: true });
      const tailwindCss = `@tailwind base;
@tailwind components;
@tailwind utilities;
`;
      await fs__namespace.writeFile(path__namespace.join(stylesDir, "tailwind.css"), tailwindCss, "utf-8");
      console.log("[ProjectManager] Created src/styles/tailwind.css");
      const cssDir = path__namespace.join(projectPath, "wwwroot", "css");
      await fs__namespace.mkdir(cssDir, { recursive: true });
      console.log("[ProjectManager] Generating initial Tailwind CSS...");
      await execa2("npm", ["run", "build:css"], {
        cwd: projectPath
      });
...
`;
  ...
  async createElectronFileManagerTemplate(projectPath, projectName) {
    const { execa: execa2 } = await import("execa");
    const templateDir = path__namespace.join(__dirname, "..", "templates", "electron-filemanager");
    const templateMetadata = JSON.parse(
      await fs__namespace.readFile(path__namespace.join(templateDir, "template.json"), "utf-8")
    );
    console.log(`[ProjectManager] Creating ${templateMetadata.name} template...`);
    for (const fileConfig of templateMetadata.files) {
      const sourcePath = path__namespace.join(templateDir, fileConfig.source);
      const destPath = path__namespace.join(projectPath, fileConfig.destination);
      await fs__namespace.mkdir(path__namespace.dirname(destPath), { recursive: true });
      let content = await fs__namespace.readFile(sourcePath, "utf-8");
      ...
      await fs__namespace.writeFile(destPath, content, "utf-8");
    ...
    if (templateMetadata.postInstall) {
      for (const command of templateMetadata.postInstall) {
        ...
        await execa2(command.command, command.args, {
          cwd: path__namespace.join(projectPath, command.cwd || ".")
        });
      }
    }
    if (templateMetadata.scripts) {
      ...
      await fs__namespace.writeFile(
        path__namespace.join(projectPath, "package.json"),
...
class TranspilerService {
  ...
  async transpileFile(tsxPath) {
    ...
    try {
      ...
      const ast = libExports.parse(tsxContent, {
        sourceType: "module",
      ...
      const result = await babel__namespace.transformFromAstAsync(ast, tsxContent, {
        filename: tsxPath,
      ...
      try {
        const keysFileExists = await fs__namespace.access(keysFilePath).then(() => true).catch(() => false);
        if (keysFileExists) {
          const keysContent = await fs__namespace.readFile(keysFilePath, "utf-8");
          await fs__namespace.writeFile(tsxPath, keysContent, "utf-8");
  ...
  async generateTailwindCss(projectPath) {
    const startTime = Date.now();
    try {
      const tailwindConfigPath = path__namespace.join(projectPath, "tailwind.config.js");
      const inputCssPath = path__namespace.join(projectPath, "src/styles/tailwind.css");
      const outputCssPath = path__namespace.join(projectPath, "wwwroot/css/tailwind.css");
      const configExists = await fs__namespace.access(tailwindConfigPath).then(() => true).catch(() => false);
      if (!configExists) {
      ...
      const inputExists = await fs__namespace.access(inputCssPath).then(() => true).catch(() => false);
      if (!inputExists) {
      ...
      await fs__namespace.mkdir(path__namespace.dirname(outputCssPath), { recursive: true });
      ...
      const stats = await fs__namespace.stat(outputCssPath);
...
function registerProjectHandlers(projectManager2) {
  electron.ipcMain.handle("project:create", async (_, projectPath, template, options) => {
    try {
      const project = await projectManager2.createProject(projectPath, template, options);
...
function registerTranspilerHandlers(transpilerService2) {
  ...
  electron.ipcMain.handle("transpiler:generateTailwindCss", async (_, projectPath) => {
    try {
      const result = await transpilerService2.generateTailwindCss(projectPath);
      return { success: result.success, data: result };
    } catch (error) {
      return { success: false, error: error.message };
    }
