(function() {
  ...
  const posix = {
    ...
    parse(path) {
      ...
      if (end !== -1) {
        const start22 = startPart === 0 && isAbsolute ? 1 : startPart;
        if (startDot === -1 || // We saw a non-dot character immediately before the dot
        preDotState === 0 || // The (right-most) trimmed path component is exactly '..'
        preDotState === 1 && startDot === end - 1 && startDot === startPart + 1) {
          ret.base = ret.name = path.slice(start22, end);
        } else {
          ret.name = path.slice(start22, startDot);
          ret.base = path.slice(start22, end);
          ret.ext = path.slice(startDot, end);
        }
      }
      if (startPart > 0) {
